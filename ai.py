...
class MCTSNode:
	def __init__(self, board: Board, turn: str, parent: MCTSNode | None = None, move: dict[str, Any] | None = None) -> None:
...
class AI:
	def __init__(self, color: str, actualAI: AIEnum) -> None:
		self.actualAI = actualAI
		# 'color' is the color this AI will play with (B or W)
		self.color: str = color

	def getColouredMessage(self, msg:str):
		return f"{GREEN_BACKGROUND}{YELLOW_TEXT}{msg}{RESET}" if self.actualAI == AIEnum.MCTS else f"{BLUE_BACKGROUND}{WHITE_TEXT}{msg}{RESET}"
	...
	def get_move(self, current_board: Board) -> dict[str, int]:
		if(self.actualAI == AIEnum.MCTS):
			...
			print(self.getColouredMessage("[MCTS] => Nova posição definida!"))
			print(move)
			return move
		else:
